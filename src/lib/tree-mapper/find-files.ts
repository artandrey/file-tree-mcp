...
export async function findFiles(
  ...
  try {
    ...
    const filteredPaths = gitignore
      ? filePaths.filter((filePath: string) => {
    ...
    return [
      ...otherFiles.sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' })), // Sort case-insensitively
      ...baseLevelFiles.sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' })), // Sort case-insensitively
    ];
  } catch (error) {
